with open('terminalout.txt', 'w') as term, open('log.txt', 'w') as logb:
    ...
    if(isWindows()): 
        PATHCSVFOLDER= ABSPATH+"\\stock\\test" #path per windows
    else: PATHCSVFOLDER= ABSPATH+"/stock/test" #path per unix
    ...
    # BOUND_LOW, BOUND_UP = 0, 10 # 0,10
    NDIM = len(stockdf) #dimensione singola tupla default 30 # lunghezza portafoglio (numero di azioni disponibili)

    MU = 8 #generazione tuple population, deve essere multiplo di 4 (Dimensione popolazione)
